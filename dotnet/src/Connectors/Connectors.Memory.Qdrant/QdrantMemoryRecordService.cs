    ...
    where TDataModel : class
{
    ...
    private readonly IMemoryRecordMapper<TDataModel, PointStruct> _mapper;
    ...
    public QdrantMemoryRecordService(QdrantClient qdrantClient, string defaultCollectionName, QdrantMemoryRecordServiceOptions<TDataModel>? options = null)
    {
        ...
        // Assign Mapper.
        if (this._options.MapperType == QdrantMemoryRecordMapperType.QdrantPointStructCustomMapper)
        {
            if (this._options.PointStructCustomMapper is null)
            {
                throw new ArgumentException($"The {nameof(QdrantMemoryRecordServiceOptions<TDataModel>.PointStructCustomMapper)} option needs to be set if a {nameof(QdrantMemoryRecordServiceOptions<TDataModel>.MapperType)} of {nameof(QdrantMemoryRecordMapperType.QdrantPointStructCustomMapper)} has been chosen.", nameof(options));
            }
        ...
        {
            this._mapper = new QdrantMemoryRecordJsonMapper<TDataModel>(new QdrantMemoryRecordJsonMapperOptions { HasNamedVectors = this._options.HasNamedVectors });
        }
    }
    ...
    /// <inheritdoc />
    async Task<Guid> IMemoryRecordService<Guid, TDataModel>.UpsertAsync(TDataModel record, UpsertRecordOptions? options, CancellationToken cancellationToken)
    {
        Verify.NotNull(record);
    ...
    /// <inheritdoc />
    async IAsyncEnumerable<Guid> IMemoryRecordService<Guid, TDataModel>.UpsertBatchAsync(IEnumerable<TDataModel> records, UpsertRecordOptions? options, [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        Verify.NotNull(records);
