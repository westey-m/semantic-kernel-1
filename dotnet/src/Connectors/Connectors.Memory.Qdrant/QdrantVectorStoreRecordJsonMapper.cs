...
internal sealed class QdrantVectorStoreRecordJsonMapper<TRecord> : IVectorStoreRecordMapper<TRecord, PointStruct>
    where TRecord : class
{
    ...
    private readonly QdrantVectorStoreRecordJsonMapperOptions _options;

    /// <summary>
    /// Initializes a new instance of the <see cref="QdrantVectorStoreRecordJsonMapper{TDataModel}"/> class.
    /// </summary>
    /// <param name="options">Optional options to use when doing the model conversion.</param>
    public QdrantVectorStoreRecordJsonMapper(QdrantVectorStoreRecordJsonMapperOptions? options)
    {
        this._options = options ?? new QdrantVectorStoreRecordJsonMapperOptions();
