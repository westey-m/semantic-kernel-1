...
/// </summary>
public class RedisVectorDBRecordServiceOptions<TDataModel>
    where TDataModel : class
{
    ...
    /// </summary>
    public RedisVectorDBRecordMapperType MapperType { get; init; } = RedisVectorDBRecordMapperType.Default;

    /// <summary>
    /// Gets or sets an optional custom mapper to use when converting between the data model and the redis record.
    /// </summary>
    /// <remarks>
    /// Set <see cref="MapperType"/> to <see cref="RedisVectorDBRecordMapperType.JsonNodeCustomMapper"/> to use this mapper."/>
    /// </remarks>
    public IVectorDBRecordMapper<TDataModel, (string Key, JsonNode Node)>? JsonNodeCustomMapper { get; init; } = null;
}
