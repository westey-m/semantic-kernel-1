...
internal sealed class RedisMemoryRecordMapper<TConsumerDataModel> : IMemoryRecordMapper<TConsumerDataModel, (string Key, JsonNode Node)>
    where TConsumerDataModel : class
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="RedisMemoryRecordMapper{TConsumerDataModel}"/> class.
    /// </summary>
    /// <param name="keyFieldJsonPropertyName">The name of the key field on the model when serialized to json.</param>
    public RedisMemoryRecordMapper(string keyFieldJsonPropertyName)
    {
