...
/// <summary>
/// Interface for accessing the list of collections in a vector store, which also exposes methods to make calls with automatic embedding generation.
/// </summary>
/// <remarks>
...
public interface ITextEmbeddingVectorStore : IVectorStore
{
    /// <summary>
    /// Get a collection from the vector store, which also exposes methods to make calls with automatic embedding generation.
    /// </summary>
    ...
    new ITextEmbeddingVectorStoreRecordCollection<TKey, TRecord> GetCollection<TKey, TRecord>(string name, VectorStoreRecordDefinition? vectorStoreRecordDefinition = null)
        where TKey : notnull
        where TRecord : class;
}
